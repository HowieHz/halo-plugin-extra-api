...
[![Halo Version](https://img.shields.io/badge/Halo-2.26.0+-brightgreen.svg)](https://halo.run)
...
2. **每次更新都需要手动选择全量版**: Halo 应用商店默认下载的是轻量版；如果您正在使用全量版，不能直接点击默认更新，请每次更新都前往应用商店手动选择与当前平台匹配的全量版版本。
3. **正确的更新流程**:
    - 方法一：停止插件 → 卸载插件 → **重启 Halo** → 安装新版本 → 启动插件
    - 方法二：停止插件 → 卸载插件 → 安装新版本 → **重启 Halo** → 启动插件
4. **卸载推荐做法**: 在卸载全量版后**重启 Halo** 确保原生库资源完全释放。
