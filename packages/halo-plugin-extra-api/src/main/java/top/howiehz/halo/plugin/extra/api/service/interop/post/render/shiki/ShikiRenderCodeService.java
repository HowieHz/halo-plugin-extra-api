package top.howiehz.halo.plugin.extra.api.service.interop.post.render.shiki;
...
import top.howiehz.halo.plugin.extra.api.service.core.config.ShikiConfig;
...
public class ShikiRenderCodeService {
    ...
    private Map<String, String> processRequestsIntelligently(List<HighlightRequest> allRequests) {
        ...
        int poolMaxSize = getRuntimePoolMaxSize();
        int numGroups = Math.min(requestsToRender.size(), poolMaxSize);

        log.debug("智能分组: {} 个请求分配到 {} 个引擎(池大小: {})", requestsToRender.size(),
            numGroups, poolMaxSize);
    ...
    private int getRuntimePoolMaxSize() {
        try {
            return Math.max(1, extensionGetter.getEnabledExtension(NodeRuntime.class)
                .flatMap(NodeRuntime::stats)
                .map(stats -> stats.poolMaxSize())
                .blockOptional()
                .orElse(1));
        } catch (Exception e) {
            log.warn("Failed to read node runtime stats: {}", e.getMessage());
            return 1;
        }
    }
