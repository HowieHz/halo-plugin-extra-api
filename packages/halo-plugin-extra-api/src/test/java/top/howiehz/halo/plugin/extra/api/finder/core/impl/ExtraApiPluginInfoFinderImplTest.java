 ...
 * 测试在完整版环境下的行为（因为测试时 Shiki Node module provider 类存在）
 */
class ExtraApiPluginInfoFinderImplTest {
    ...
    @Test
    void shouldDetectFullVersionWhenNodeModuleProviderExists() {
        // 在测试环境中，Shiki Node module provider 类应该存在，所以应该检测为完整版
        Boolean result = finder.isFullVersion().block();
        assertTrue(result, "Should detect full version when Node module provider class exists");
    }

    @Test
    void shouldDetectNotLiteVersionWhenNodeModuleProviderExists() {
        Boolean result = finder.isLiteVersion().block();
        assertFalse(result, "Should not detect lite version when Node module provider class exists");
    }
